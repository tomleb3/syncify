...
Also supports refresh-token-only reauthorization.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Interactive setup for Syncify.')
    parser.add_argument(
        '--auth-only',
        action='store_true',
        help='Refresh the Spotify authorization and push only SPOTIFY_REFRESH_TOKEN.',
    )
    return parser.parse_args()


def _prompt_spotify_credentials() -> tuple[str, str]:
    client_id = os.environ.get('SPOTIFY_CLIENT_ID') or prompt('Spotify Client ID')
    client_secret = os.environ.get('SPOTIFY_CLIENT_SECRET') or prompt_secret('Spotify Client Secret')
...
def _authorize_and_fetch_playlists(client_id: str, client_secret: str) -> tuple[str, str, list[dict]]:
    ...
    return access_token, refresh_token, all_playlists


def _prompt_repo(detected_repo: str, auth_only: bool) -> str:
    if auth_only:
        return prompt('GitHub repo to push refresh token to (owner/repo)', detected_repo)
    return prompt('GitHub repo to push secrets/variables to (owner/repo)', detected_repo)


def _push_refresh_token_only(repo: str, refresh_token: str) -> None:
    print(f'\nPushing refresh token to {repo}...')
    _gh_push(
        repo=repo,
        secrets={'SPOTIFY_REFRESH_TOKEN': refresh_token},
        variables={},
    )


def _run_full_setup(client_id: str, client_secret: str, access_token: str, refresh_token: str, all_playlists: list[dict]) -> None:
    ...
    repo = _prompt_repo(detected_repo, auth_only=False)
...
def main() -> None:
    ...
    client_id, client_secret = _prompt_spotify_credentials()
    access_token, refresh_token, all_playlists = _authorize_and_fetch_playlists(client_id, client_secret)

    if args.auth_only:
        detected_repo = _detect_gh_repo()
        repo = _prompt_repo(detected_repo, auth_only=True)
        if repo:
            _push_refresh_token_only(repo, refresh_token)
        else:
            print(f'SPOTIFY_REFRESH_TOKEN (add to GitHub Secrets manually):\n  {refresh_token}')
        print('\nRe-run `make setup` if you want to change playlist selection or sync mode.')
    else:
        _run_full_setup(client_id, client_secret, access_token, refresh_token, all_playlists)
